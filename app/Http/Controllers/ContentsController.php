...
class ContentsController extends Controller
{
    ...
    {
		$exceptTranslateUrl = ['pay', 'paypal-pay'];
		...
		if(!in_array($slug, $exceptTranslateUrl)) {
			$translateUrl = db("translate")->where([
				'dil'=> App::getLocale(),
				'ceviri' => $slug
			])->first();
			if($translateUrl) {
				if($translateUrl->icerik !="") {
					$slug = $translateUrl->icerik;
				}
			} else {
				$translateUrl = db("translate")->where([
					'ceviri' => $slug
		...
		if(is_null($c)) {
			$c = db("contents")->where("slug", $slug)->first();
